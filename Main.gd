...
var TurretEnemy = preload("res://actors/Apprentice.tscn")
...
var hspread = 100  # horizontal spread (in pixels)
...
func cleanUp():
	...
	for portal in $Portal.get_children():
		portal.queue_free()
...
func _draw():
	...
	#for drawing the rooms(only for reference)
#	for room in $Rooms.get_children():
#		draw_rect(Rect2(room.position - room.size, room.size * 2),
#				 Color(0, 1, 0), false)

	#for drawing the paths(for reference)
#	if path:
...
#		#placing enemiess
		if(room!=start_room && room!=end_room):
			...
			enemy.position = Vector2(int(rand_range(room.position.x, room.position.x-room.size.x)), int(rand_range(room.position.y, room.position.y-room.size.y)))
			$Enemies.add_child(enemy)
		
		if(randi()%2==0 && room!=start_room && room!=end_room):
			var turretEnemy = TurretEnemy.instance()
			turretEnemy.position = room.position
			$Enemies.add_child(turretEnemy)
			
		#placing the portal
		if(room==end_room):
			var port = portal.instance()
			port.position = room.position
			$Portal.add_child(port)
